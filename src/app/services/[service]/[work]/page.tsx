...
export default async function WorkPage({ params }: { params: Params }) {
  ...
  // Sibling works in the same service, excluding the current one.
  const otherWorks = getServiceWorks(service).filter((x) => x.slug !== work);
  ...
  return (
    <>
      ...
      {/* Related work in the same service */}
      {otherWorks.length > 0 && (
        <section className="bg-gray-50 border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
            <h2 className="text-2xl md:text-3xl font-semibold text-gray-900 mb-10">
              More {(svc?.shortTitle ?? svc?.title ?? 'related').toLowerCase()} work
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {otherWorks.map((o) => (
                <WorkCard key={o.slug} service={service} work={o} />
              ))}
            </div>
          </div>
        </section>
